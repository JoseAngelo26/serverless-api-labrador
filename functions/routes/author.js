...
router.post('/', async (req, res) => {
    try {
        ...
        //check if the author's name already exist
...
router.patch('/:id',getAuthor, async (req,res) => {
    try {
        ...
        const upadatedAuthor = await res.author.save();
        res.json(upadatedAuthor);
    } catch (err) {
        res.status(400).json({ message: err.message});
    }
});
...
router.put('/:id', getAuthor, async (req,res) => {
    try{
        const upadatedAuthor = await AuthorModel.findByIdAndUpdate(
            req.params.id,
            req.body,
            { new: true }
        )
